...
/**
 * A repository over one in-memory `UserDataBundle`.
 *
 ...
 *
 * Durability is the store's problem, not this class's: demo mode hands it a
 * bundle in server memory, the desktop build hands it one backed by a JSON
 * file, and the query, cascade and ordering logic below is identical either
 * way.
 */
export class BundleRepository implements ApplyPilotRepository {
  constructor(
    readonly kind: 'demo' | 'file',
    private readonly store: BundleStore,
  ) {}

  /** The bundle, for reading. */
  private data(): UserDataBundle {
    return this.store.read();
  }

  /**
   * The bundle, for changing.
   *
   * Method bodies here are synchronous: they mutate what this returns and then
   * return. So the commit is queued rather than run inline — by the time it
   * fires, the change it exists to persist has actually happened. Callers that
   * change something use this instead of `data()`, which is the only thing they
   * have to remember for a durable store to keep up.
   */
  private mutable(): UserDataBundle {
    const bundle = this.store.read();
    queueMicrotask(() => this.store.commit());
    return bundle;
  }
  ...
  async createCollege(userId: string, input: NewCollege): Promise<College> {
    ...
    this.mutable().colleges.push(college);
  ...
  async createRecommender(userId: string, input: NewRecommender): Promise<Recommender> {
    ...
    this.mutable().recommenders.push(recommender);
  ...
  async createScholarship(userId: string, input: NewScholarship): Promise<Scholarship> {
    ...
    this.mutable().scholarships.push(scholarship);
  ...
  async createCoachSession(userId: string, input: NewCoachSession): Promise<CoachSession> {
    ...
    this.mutable().coachSessions.push(session);
  ...
  /*
   * These two take `userId` to match the interface even though a bundle holds
   * exactly one user's records: emptying the bundle and deleting that user's
   * data are the same act. Keeping the signature identical means callers
   * cannot tell the adapters apart, which is the point of the seam.
   */
